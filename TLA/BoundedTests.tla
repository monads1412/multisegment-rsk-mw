...
\* ============================================================
\* Comprehensive bounded tests for FormalizedCorollary.tla
\*
\* This module is a DROP-IN replacement for the previous
\* BoundedTests.tla.  The existing BoundedTests.cfg and
\* run_bounded_tests.sh can stay unchanged.
\*
\* Besides the previous Lemma/Corollary checks, this version
\* independently checks the main intermediate constructions:
\*   - bounded input universe
\*   - depth and depth fibers
\*   - admissible enumerations
\*   - j_k and distinguished/remaining indices
\*   - i |-> i^vee
\*   - transformed segments and l(m)
\*   - derived multisegment m'
\*   - min(m)
\*   - leading sequence (3.1a--3.1c)
\*   - I*, Delta_i*, m^cross, Delta^circle
\*   - K and MW packaging
\*   - Lemma 1 and all three Corollary 3.4 formulations
\*
\* BoundedUniverseCheck intentionally includes ALL structural
\* checks so that the old .cfg file automatically runs them.
\* ============================================================

CONSTANTS EndpointBound, MaxSegments

\* ============================================================
\* Finite input universe
\* ============================================================
...
BoundedSegments ==
    {segmentValue \in (EndpointValues \X EndpointValues) :
        segmentValue[1] <= segmentValue[2]}
...
\* ============================================================
\* One TLC initial state for each bounded multisegment
\* ============================================================
...
\* Optional alternative initial predicate.  If a config uses this
\* instead of Init, TLC counts only NON-VACUOUS Corollary cases.
...
InputUniverseCheck ==
    /\ Len(TestM) \in 1..MaxSegments
    /\ \A indexValue \in IndexSet(TestM) :
        TestM[indexValue] \in BoundedSegments
...
\* IMPORTANT: the existing BoundedTests.cfg already contains the
\* invariant name BoundedUniverseCheck.  Folding AllDefinitionChecks
\* into it means the old config and shell script automatically run
\* every structural check above, with no changes required.
